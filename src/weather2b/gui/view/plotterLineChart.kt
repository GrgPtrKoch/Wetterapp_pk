...
object plotterLineChart {
    ...
    private val series: MutableList<XYChart.Series<String, Number>> = mutableListOf()
    private val yAxis = NumberAxis()
    private val weather = Gui.Companion.selectedLocationWeather?.getDailyList()
    ...
    fun getView(): LineChart<String?, Number?> {
        ...
        // Bei X- und Y-Achse Labels die gewuenschte Schriftgroesse von weather2b.gui.design.appStyle verwenden
        Platform.runLater {
    ...
    private fun updateSerie(serie: XYChart.Series<String, Number>, value: Int) {
        Platform.runLater {
            ...
            label.style =
                "-fx-font-size: 11px; -fx-font-weight: bold; -fx-opacity: 0.65;-fx-font-family: 'Helvetica'; -fx-text-fill: ${if (serie == series[0]) "brown" else "black"}; -fx-border-width: 0; -fx-background-color: white;"
            data.node = label
            // bis hier
